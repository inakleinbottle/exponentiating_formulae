...
int main()
{
	...
	for (auto x = env.sbasis.begin(), e = env.sbasis.end(); x != e; x = env.sbasis.nextkey(x))
		std::cout << STENSOR(x, 1) << "\t"
		<< k(STENSOR(x, 1), L) << "\n";
    ...
    for (auto x = env.sbasis.begin(), ex = env.sbasis.end(); x != ex; x = env.sbasis.nextkey(x))
        for (auto y = env.sbasis.begin(), ey = env.sbasis.end(); y != ey; y = env.sbasis.nextkey(y)) {
            STENSOR z = alg::half_shuffle_multiply(STENSOR(x, 1), STENSOR(y, 1));
